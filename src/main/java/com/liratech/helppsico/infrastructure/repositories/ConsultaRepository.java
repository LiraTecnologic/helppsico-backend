...
public interface ConsultaRepository extends JpaRepository<ConsultaEntity, UUID> {
    ...
    @Query("SELECT c FROM Consulta c WHERE c.finalizada = false AND c.psicologo.id = :idPsicologo AND c.paciente.id = :idPaciente")
    Page<ConsultaEntity> consultarConsultasFuturas(UUID idPsicologo, UUID idPaciente);

    @Query("SELECT c FROM Consulta c WHERE c.finalizada = true AND c.psicologo.id = idPsicologo AND c.paciente.id = :idPaciente")
    Page<ConsultaEntity> consultarHistorico(UUID idPsicologo, UUID idPaciente);
}
